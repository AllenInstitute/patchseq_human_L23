...
# Pixel to um conversion (100um ~ 165 pixels)
convert = setNames(c(100/165,100/330,100/330),c("case_set_1","case_set_2","case_set_3"))
...
#[1] "Average L23 depth for case_set_1 = 281"
#[1] "Average L23 depth for case_set_2 = 223"
#[1] "Average L23 depth for case_set_3 = 257"
...
pdf("cell_size_vs_depth_mouse.pdf",height=7,width=7)
par(mar=c(6,6,6,6))
...
## LOAD THE HUMAN DATA FOR COMPARISON WITH MOUSE
...
print(paste("Average human / mouse cross-sectional area (L2):",signif(l2dex[1,"area"]/l2dex[2,"area"],2)))
print(paste("Average human / mouse cross-sectional area (deep L3):",signif(l3dex[1,"area"]/l3dex[2,"area"],2)))
# [1] "Average human / mouse cross-sectional area (L2): 2.1"
# [1] "Average human / mouse cross-sectional area (deep L3): 3.0"

l2dex <- data_summary(dens[dpt >= -0.15,],"het",c("species"))
l3dex <- data_summary(dens[(dpt >= -0.95)&(dpt < -0.7),],"het",c("species"))
print(paste("SD human / mouse cross-sectional area (L2):",signif(l2dex[1,"het"]/l2dex[2,"het"],2)))
print(paste("SD human / mouse cross-sectional area (deep L3):",signif(l3dex[1,"het"]/l3dex[2,"het"],2)))
#[1] "SD human / mouse cross-sectional area (L2): 2.4"
#[1] "SD human / mouse cross-sectional area (deep L3): 4.0"
...
p <- ggplot(df2, aes(x=group, y=dens, group=species, color=species)) + 
  geom_line(size=2.5) +
  geom_point()+
  scale_color_manual(values=c("#4D8000","#A84920")) +
...
ggsave("cell_density_vs_depth.pdf",height=7,width=7)
...
ggsave("cell_area_vs_depth.pdf",height=7,width=7)
...
p + labs(title="Cell heterogeneity", x="Scaled depth", y = "Cell heterogeneity") + 
	...
	theme_classic() + 
	geom_hline(yintercept=0)
ggsave("cell_heterogeneity_vs_depth.pdf",height=7,width=7)


## Print the absolute density
dh = 40*cellsPerMMh/1000 
dm = 40*cellsPerMM/1000
print(paste("Mouse density (mm^3) =",signif(mean(dm),3),"+/-",signif(sd(dm),3)))
#[1] "Mouse density (mm^3) = 165 +/- 24.9"
print(paste("Human density (mm^3) =",signif(mean(dh),3),"+/-",signif(sd(dh),3)))
#[1] "Human density (mm^3) = 27.7 +/- 4.46"

## Plot the absolute density
cpm <- data.frame(species=c(rep("Human",5),rep("Mouse",3)),density=c(dh,dm))
...
p <- ggplot(cpm, aes(y=density, x=species)) + 
  ...
  scale_fill_manual(values=c("#4D8000","#A84920")) +
  geom_point(color = "black", shape="-", size=7) + 
  ylim(0,200) 
p

ggsave("absolute_cell_density.pdf",height=7,width=3)

print(l2)
#[1] -0.2461444 -0.2049731 -0.3252786
print(l2H)
